...
function Cat() {
  ...
  const fetchAllCats = async () => {
    try {
      const response = await axios.get("http://127.0.0.1:8000/api/products/cat");
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/api/products/cat?search=${searchTerm}`);
  ...
  return (
    <div style={{ textAlign: 'center', margin: '20px' }}>
      <div>
        ...
        <div style={{ marginBottom: '20px', padding: '18px' }}>
          <div style={{ display: 'inline-block', marginLeft: '20px', margin: '10px' }}>
            ...
            <button onClick={fetchAllCats} style={{ marginLeft: '10px', padding: '8px 16px', backgroundColor: '#1976D2', color: '#FFF', borderRadius: '5px', border: '1px solid white', cursor: 'pointer' }}>
              Show All
            </button>
          </div>
        </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap' }}>
        {cats.map((cat) => (
          <div key={cat.id} style={{ margin: '10px', padding: '10px', width: '200px', backgroundColor: '#FFFAF0', boxShadow: '0px 2px 4px rgba(0, 0, 0, 0.1)', borderRadius: '5px' }}>
            <img
              src={cat.img}
              alt=""
            ...
            <div style={{ padding: '10px' }}>
              <h3 style={{ margin: '0', fontSize: '16px' }}>{cat.name}</h3>
              <p style={{ fontSize: '14px', margin: '5px 0', color: '#333' }}>{cat.description}</p>
              <p style={{ fontSize: '14px', fontWeight: '600', margin: '5px 0', color: '#1976D2' }}>Price: {cat.price}</p>
              <p style={{ fontSize: '14px', fontWeight: '550', margin: '5px 0', color: '#333' }}>Stock: {cat.stock}</p>
