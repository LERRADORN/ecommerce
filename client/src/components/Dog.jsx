...
function Dog() {
  ...
  const fetchAllDogs = async () => {
    try {
      const response = await axios.get("http://127.0.0.1:8000/api/products/dog");
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/api/products/dog?search=${searchTerm}`);
  ...
  return (
    <div style={{ textAlign: 'center', margin: '20px' }}>
      ...
      <h1 style={{ padding: '20px 0px 3px 0px', color: '#FFFAF0' }}>Dog Products</h1>
        <div style={{ marginBottom: '20px', padding: '18px'}}>
          <div style={{ display: 'inline-block', marginLeft: '20px', margin: '10px' }} >
            ...
            <button onClick={fetchAllDogs} style={{ marginLeft: '10px', padding: '8px 16px', backgroundColor: '#1976D2', color: '#FFF', borderRadius: '5px', border: '1px solid white', cursor: 'pointer' }}>
              Show All
            </button>
          </div>
        </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap' }}>
        {dogs.map((dog) => (
          <div key={dog.id} style={{ margin: '10px', padding: '10px', width: '200px', backgroundColor: '#FFFAF0', boxShadow: '0px 2px 4px rgba(0, 0, 0, 0.1)', borderRadius: '5px' }}>
            <img
              src={dog.img}
              alt=""
            ...
            <div style={{ padding: '10px' }}>
              <h3 style={{ margin: '0', fontSize: '16px' }}>{dog.name}</h3>
              <p style={{ fontSize: '14px', margin: '5px 0', color: '#333' }}>{dog.description}</p>
              <p style={{ fontSize: '14px', fontWeight: '600', margin: '5px 0', color: '#1976D2' }}>Price: {dog.price}</p>
              <p style={{ fontSize: '14px', fontWeight: '550', margin: '5px 0', color: '#333' }}>Stock: {dog.stock}</p>
