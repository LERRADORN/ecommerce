...
//   const navigate = useNavigate(); // Initialize useNavigate hook
  ...
  const handleAddToCart = (productId) => {
    // Redirect to cart page with product ID as URL parameter
    ...
    <div style={{ textAlign: 'center', margin: '20px' }}>
        ...
        <h1 style={{ padding: '20px 0px 3px 0px', color: '#FFFAF0' }}>All Products</h1>
            <div style={{ marginBottom: '20px', padding: '18px'}}>
                <div style={{ display: 'inline-block', marginLeft: '20px', margin: '10px' }}>
                    ...
                    <button onClick={fetchAllProducts} style={{ marginLeft: '10px', padding: '8px 16px', backgroundColor: '#1976D2', color: '#FFF', borderRadius: '5px', border: '1px solid white', cursor: 'pointer' }}>
                    Show All
                    </button>
                </div>
            </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap' }}>
        {products.map((product) => (
          <div key={product.id} style={{ margin: '10px', padding: '10px', width: '200px', backgroundColor: '#FFFAF0', boxShadow: '0px 2px 4px rgba(0, 0, 0, 0.1)', borderRadius: '5px' }}>
            <img
              src={product.img}
              alt=""
            ...
            <div style={{ padding: '10px' }}>
              ...
              <p style={{ fontSize: '14px', margin: '5px 0', color: '#333' }}>{product.description}</p>
              <p style={{ fontSize: '14px', fontWeight: '600', margin: '5px 0', color: '#1976D2' }}>Price: {product.price}</p>
              <p style={{ fontSize: '14px', fontWeight: '550', margin: '5px 0', color: '#333' }}>Stock: {product.stock}</p>
